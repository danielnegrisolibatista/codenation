...
public class Estacionamento {
	...
	/*
		List<Carro> carroLista = getCarroLista();
		Carro carroEstacionado = null;
		
		if (carroLista.size() > 0) {
			carroEstacionado = carroLista
					.stream()
					.filter(carroDaLista -> 
					        carroDaLista.getCor() == carro.getCor() &&
							carroDaLista.getPlaca() == carro.getPlaca() &&
							carroDaLista.getMotorista().getHabilitacao() == carro.getMotorista().getHabilitacao()
							)
	                .findFirst()
	                .orElse(null);
		}
